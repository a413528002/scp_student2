import { grab, queryLogs } from '@/services/student/gloan.js';
...
const GrabLoanModel = {
  ...
  effects: {
    ...
    * countDown({payload}, {call, put, select}) {
      while (true) {
        const startDuration = yield select(state => state.studentGrabLoan.startDuration);
        if (startDuration <= 0) {
          break;
        }
