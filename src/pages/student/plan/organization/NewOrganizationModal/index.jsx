...
const NewOrganizationModal = (props) => {
  ...
  return (
    ...
    >
      ...
      <div>支行：各机构可选择在各区域建立支行，第1期需在总行所在的区域建立支行；建设支行200万，租赁支行50万；</div>
      <div>每个支行可以抢1单存款</div>
      <p/>
      <Space>
        <Button htmlType="button" onClick={handleNewOrganizationCancelModal}>
          取消
        </Button>
        <Button type="primary" htmlType="submit" loading={loading} onClick={() => createOrganization('L')}>
          租赁
        </Button>
        <Button type="primary" htmlType="submit" loading={loading} onClick={() => createOrganization('B')}>
          建设
        </Button>
      </Space>
    </Modal>
